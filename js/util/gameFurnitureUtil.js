...
gameFurnitureInitialize = (gameFurniture) => {
...
gameFurnitureDisplay = (gameFurniture, gameObjFurniture) => {
    for (i = 0; i < gameFurniture.length; i++) {
        for (j = 0; j < gameFurniture[i].length; j++) {
            if (i == 0 && j == 0) continue;
            if (gameFurniture[i][j] != 0 && gameFurniture[i][j] != 9 && gameObjFurniture[i][j] != null) {
...
definePosByObjType = (g, i, j) => {
    ...
    case 1:
        x = j * tile_w - ((g.states.stateImage[g.currState].height - floor_h));
        y = i * tile_h - ((g.states.stateImage[g.currState].height - floor_h));
        p.set(x, y);
        break;
    //double bed
    case 2:
        x = j * tile_w - ((g.states.stateImage[g.currState].width - tile_w)/2);
        y = i * tile_h; 
        //turned to left
        if (g.currState == 1) { 
            x += (tile_w/2);
    ...
    //single bed
    case 3:
        x = j * tile_w - ((g.states.stateImage[g.currState].width - tile_w)/2) - 30;
        y = i * tile_h - 10; 
        //turned to left
        if (g.currState == 1) { 
            x += (tile_w/2) + 30;
    ...
    case 4:
        x = j * tile_w - ((g.states.stateImage[g.currState].height - floor_h));
        y = i * tile_h - ((g.states.stateImage[g.currState].height - floor_h)) + tile_h;
        if (g.currState == 1) { 
            x += (floor_h/2);
            y -= (tile_w/2);
        }  
...
insertObject = (i, j, g) => {
    if (hasSpaceInsert(g.states.tilesStates[g.currState].x, g.states.tilesStates[g.currState].y, i, j)) {
        setFurniture(g, i, j);
        OBJ_SEL_SHOW = false;
        objSelMenu = null;
        selItem.set(i, j);
